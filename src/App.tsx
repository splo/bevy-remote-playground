...
import type { OpenRpcBrpBridge } from './lib/useBrp'
...
function App() {
  ...
  const handleLoad = useCallback(async (source: AppSource) => {
    ...
    const bridge = await loadBridge(source.jsUrl, source.wasmUrl) as OpenRpcBrpBridge | null
    if (!bridge) {
      cleanup()
    ...
    try {
      const result = await bridge['rpc.discover']()
